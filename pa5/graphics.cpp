...
int window::moveBy = 4;
shared_ptr<shape> window::curr_obj = nullptr; 
...
void object::draw(size_t count) {
   //if center off screan put it back on
   //rubberband it
   if(center.xpos <= 0){
      center.xpos = window::get_width()-1; 
   }
      ...
      center.ypos = window::get_height()-1;
   }
   pshape->draw (center, color);
   pshape->number(center,count);
}

void object::drawborder(const rgbcolor& col,
                           const GLfloat& width){
   pshape->draw_border(col, width, center);
}
...
void window::display() {
   ...
   if(objects.size() != 0){
      objects[selected_obj].drawborder(borderColor, borderWidth);
   }
   mus.draw();
   glutSwapBuffers();
}
...
void window::select_object(size_t obj){
   if(obj < window::objects.size()){
      ...
      window::curr_obj = objects[obj].get_obj();
   }
}

void window::move_selected_object(GLfloat x, GLfloat y){
